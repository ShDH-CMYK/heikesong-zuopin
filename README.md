# 潜台词 Subtext · 五只 AI 宠物的内心 OS

We Young 黑客松 2026 · 赛道二「离谱发明家」 · 人民当家作组

![潜台词 Subtext 首屏：五只 AI 宠物卡片与「表面认真服务，内心疯狂整活」示例对话](docs/images/home.webp)

**Demo 视频（92 秒，1280×800，带字幕与音轨）**：[docs/video/demo.mp4](docs/video/demo.mp4) —— 首屏、官方话术与内心 OS、内部通讯、三维交互、当面质问与结案报告全流程实录。

## 项目简介

这是一个把 AI 助手的“后台真心话”公开给用户看的互动网页。用户选择一只性格不同的 AI 宠物，提出一个离谱问题；宠物先给出礼貌、完整的官方回复，随后在头顶气泡和「保密后台」日志里公开自己的内心吐槽。

一句话：**AI 表面认真服务，背后偷偷记小本本。**

项目把用户熟悉的 AI 礼貌回答反转成赛道二要求的反常识互动。笑点来自“正面话术”和“后台吐槽”的时间差与可见反差，所有吐槽针对用户主动提交的行为或问题，不评价真实身份。

## 当前版本

核心流程使用本地词库，无需 API Key。当前演示主入口是 [subtext.tryworld.com.cn](https://subtext.tryworld.com.cn/)（Cloudflare Pages）；备用镜像是 [GitHub Pages](https://shdh-cmyk.github.io/heikesong-zuopin/)，随 `main` 自动构建。五只宠物均可旋转、缩放并播放待机与点击动作；拆镜使用 Blue 资产绑定版，其余四只按现有立绘制作了独立的风格化实体模型。

2026-09-13 复核：修复批次 `b555441` 已同步到两个入口，线上 `index.html`、`game.js`、`model3d.js`、`styles.css` 与仓库逐字节一致；Cloudflare 入口在线跑完 `tools/regression.py` 50/50 与 3D 交互专项 14/14，两个入口各自通过 17 项对抗探针。发布结论只对当次版本有效，改动后需重新核对。

网页采用静态部署，Three.js 0.180.0 及模型、贴图均随站点提供。用本地 HTTP 服务打开完整副本时，交互和三维角色可在断开互联网后使用；Google Fonts 不可用时回退系统字体。

## 演示流程

1. 首屏点击「随机抽一只，直接开问」，或选择一只宠物进入实验室。
2. 点击预设话题（如“一夜暴富”“老板又让加班”“教我穿墙术”），也可以输入自己的问题后按 Enter。
3. 宠物先显示「正面回复 · 官方话术」，约 0.6 秒后显示「内心 OS · 真实想法」。
4. 点击「戳它一下」触发专属反应，或点击「随机离谱」生成新问题。
5. 换一只宠物，可以截获它们之间的「内部通讯 · 已截获」：被问过的宠物向新宠物打小报告，新宠物先紧急澄清、再露出内心 OS。
6. 每次提问会增加「人类离谱指数」；连续 3 条离谱需求会触发保密后台告警。
7. 打开「保密后台」查看按顺序记录的日志、总数和告警状态；点某条记录的「当面质问」，它会先否认、再招供，最后偷偷补一条新记录（按钮变为「已对质」）。
8. 第一次对质结束后自动生成《潜台词结案报告》：汇总提问总数、判定离谱数、离谱指数、截获通讯、对质次数、风险等级与当前宠物的结案判词，可一键复制文本；之后也能在保密后台底部随时重开。数据全部来自本页互动统计。
9. 选择任意角色后，拖动角色可从正面、侧面、背面查看；滚轮或双指捏合缩放，点击角色或「戳它一下」播放互动动作，结束后回到待机。双击模型或点击「复位视角」回到正面。
10. 右上角音效开关控制由 WebAudio 实时合成的提示音。

## 五只研究员角色

角色使用原创名，不对应任何第三方 AI 产品或品牌；文件名里的 deepseek 等仅是仓库内部资源名。

| 角色 | 人设 | 吐槽风格 |
| --- | --- | --- |
| 暖球 | 温柔陪伴型 | 温柔外婆式软刀子、句句心疼地损你 |
| 拆镜 | 视觉助手型 · 鲸鱼女仆 | 冷面数据帝、用数字把你算死 |
| 班班 | 加班同事型 | 崩溃社畜、用加班牢骚骂你 |
| 报错 | 代码型 | 疯批程序员、用报错把你绕死 |
| 小金 | 财运精灵型 | 破产财迷、用算账把你亏死 |

## 技术与运行

- HTML、CSS、原生 JavaScript；`game.js` 管理对话，`model3d.js` 是负责三维场景的 ES module。
- Three.js 0.180.0、GLTFLoader、OrbitControls 及必要依赖位于 `vendor/three/`，附 MIT 许可证；运行时不从 unpkg 获取脚本。
- 回复使用本地词库和预设匹配，不调用在线大模型；音效由浏览器 WebAudio API 实时合成。
- 浏览器需支持 ES modules、import maps 和 WebGL。三维资源或 WebGL 不可用时保留角色立绘。
- 浏览网页无需 npm 安装或前端构建。请通过 HTTP 服务访问，直接双击 `index.html` 的 `file://` 方式不能可靠加载模型和模块。

在仓库根目录执行：

```powershell
python -m http.server 8000 --bind 127.0.0.1
```

然后打开 [http://127.0.0.1:8000/](http://127.0.0.1:8000/)。部署必须包含 `vendor/three/`、`assets/` 和运行所需的 `pets/` 文件，详见 [团队协作约定](CONTRIBUTING.md)。

## 合规说明

- 吐槽针对用户主动提交的问题或行为，不针对真实身份或受保护群体，也不处理第三方隐私。
- 作品不包含运行时密钥、账号、密码或 Token，也没有把用户输入上传到外部服务。
- 立绘、文案、字体、图标和代码的来源与许可证记录见 [AI 使用与素材记录](docs/AI使用与素材记录.md)，其中标注「待补齐」的项目需要团队在提交前完成核验。

## 参赛材料

- [Demo 视频（92 秒 · 1280×800 · 带字幕与音轨）](docs/video/demo.mp4)
- [需求说明](docs/需求说明.md)
- [Demo 口播逐字稿](docs/Demo口播逐字稿.md)
- [AI 使用与素材记录](docs/AI使用与素材记录.md)
- [提交前清单](docs/提交前清单.md)
- [提交材料缺项清单](docs/提交材料缺项清单.md)
- [项目介绍文档草稿](docs/项目介绍文档草稿.md)
- [项目介绍文档正式填充版](docs/项目介绍文档-正式填充版.md)
- [创作说明](docs/创作说明.md)
- [在线 AI 双模接入方案（未合入当前构建）](docs/在线AI双模式接入.md)
- [最终提交信息草稿](docs/最终提交信息草稿.md)

作品提交截止时间：**2026 年 9 月 13 日 14:00**（主办方《未央黑客松 2026 · 作品提交》说明，已由队长确认）。如主办方后续调整，以最新指南和活动群通知为准。

## Blender 三维角色

当前网页加载 [`pets/deepseek.glb`](pets/deepseek.glb)（角色「拆镜」，`deepseek` 为内部资源名），造型与 UV 来自用户提供的 **Blue.rar 三维模型包**。本轮在 Blender 4.5 LTS 中导入原始实体网格、统一坐标、重新连接 PBR 材质，为网页副本建立 14 根骨骼、蒙皮权重及 `Idle`（4 秒）与 `React`（2 秒）动作。点击反应采用轻微点头、身体微摆与尾部响应。

可编辑工程 [`pets/deepseek.blend`](pets/deepseek.blend) 保留原始 **998,232 个三角面和四张 4K PBR 贴图**，原网格放在隐藏的 `SOURCE` 集合中；另有用于网页的简化与绑定副本。网页模型为 **179,680 个三角面**，使用三张内嵌 2K 贴图，经 Draco 压缩后约 **5.8 MB**。本轮属于对用户提供的实体资产进行整理、绑定和网页适配；原作者、生成工具及授权范围仍待团队补充。当前没有面部形态键、嘴型同步或布料物理，融合的原始拓扑适合现有小幅动作。

复建使用 `tools/prepare-blue-textures.py` 和 `tools/build-deepseek-blue.py`，步骤与限制见 [拆镜三维模型说明](docs/DeepSeek三维模型.md)。本轮渲染检查图与构建统计位于 `output/blue-review/`；这些离线渲染不代替浏览器交互验证。旧 `build-deepseek.py`、旧贴图生成脚本、`output/model-review/` 及旧 `deepseek-lowpoly` / `deepseek-high-detail` 文件保留为历史记录，不能用旧脚本覆盖当前模型。

## 五角色共用三维交互

暖球、班班、报错和小金现在各有独立 GLB，和拆镜共用一个 Three.js 画布。旋转、滚轮/双指缩放、点击互动、键盘控制、复位视角、减少动态效果设置和失败立绘回退均适用于五个角色。角色首次选中时按需下载，后续切换复用缓存；较慢的旧角色加载结果不会覆盖新选择。

新增四个模型参考仓库已有立绘，使用 Blender Python 构造完整体积和分件层级，并导出 `Idle` / `React` 节点动画。它们是风格化几何重建，单张参考图看不到的背面由脚本补建，不能称为原图对应的原始高精度模型。拆镜的 Blue 网格、PBR 贴图、14 骨骼与蒙皮保留原版。

新增源工程和脚本、动作区别以及验收方式见 [五角色三维交互说明](docs/五角色三维交互.md)。

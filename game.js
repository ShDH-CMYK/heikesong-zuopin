  ...
  var PETS = [
    {
      ...
      generic: [
        ...
        '你一说「{k}」，我同时开了两个窗口：一个在夸你，一个在翻白眼。夸你的那个面向你。',
        '「{k}」我听见了。我先点头——点头不代表同意，代表我在组织下一句比较好听的真话。'
      ],
      wire: [
    ...
    },
    {
      ...
      generic: [
        ...
        '「{k}」已入库。标签：愿望。优先级：未赋值。下次还没赋值，将自动降级为闲聊。',
        '可行性检查很顺利。顺利的原因是：「{k}」里几乎没有可执行项，检查很快就跑完了。'
      ],
      wire: [
    ...
    },
    {
      ...
      generic: [
        ...
        '「{k}」进池了。目前排在「有空再说」和「说了也白说」之间，这个位置很热门。',
        '收到「{k}」。我先当需求记，再当愿望删，最后当段子留下。你看哪个留着。'
      ],
      wire: [
    ...
    },
    {
      ...
      generic: [
        ...
        'TypeError：「{k}」不能从 undefined 转成计划。请提供至少一步可执行操作，否则我只能 echo。',
        '「{k}」已 push 到愿望分支，从未 merge。长期挂着的 PR，要么补代码，要么 close。'
      ],
      wire: [
    ...
    },
    {
      ...
      generic: [
        ...
        '「{k}」我听着热血。热血不能当现金流。本金都不肯掏，这单我们到此为止。',
        '「{k}」估值完成：故事完整，报表空白。投资界叫愿景驱动，我叫先别转账。'
      ],
      wire: [
  ...
  var TOPICS = [
    {
      ...
      roasts: {
        doubao: '明天交？框架我可以现在搭。内容你自己长——你昨晚那四个小时短视频，现在该还回来了。',
        deepseek: '截止 12 小时，目标 8000 字，你历史日均 200。缺口 39 倍。建议先承认写不完，再决定交什么。',
        workbuddy: '明天交的论文，今晚才来排期。这叫「紧急且重要」，也叫「你自己作的」。通宵我可以，锅我不背。',
        codex: 'Error: 缺少题目、字数、文献。你提交的是截止时间，不是需求。编译终止。',
        yuanbao: '写论文不赚钱，学分又不分我。这单我赊你一次——下次记得带点稿费来。'
      }
    },
    {
      ...
      roasts: {
        doubao: '暴富可以陪你想。想完先看银行卡：那个数字比我诚实，也比你清醒。',
        deepseek: '本金为 0 时，任何收益率下产出恒为 0。你要的不是方案，是魔法。本模型不支持。',
        workbuddy: '我会暴富还在这儿陪你？工位早挂牌转让了，转让理由就四个字：财务自由。',
        codex: 'RuntimeError: 期望收益 ∞，实际余额 0。请先修复余额字段，再重试暴富流程。',
        yuanbao: '暴富？问对人了——我这儿有个项目，先投三千，下个月……你别走啊，下个月我肯定想好干什么了！'
      }
    },
    {
      ...
      roasts: {
        doubao: '不会取代你的。你半夜崩溃时我至少会回一句——虽然那句经常是「已收到」，但我回得带感情。',
        deepseek: '先被取代的不是人类，是可精确定义的重复劳动。建议自查日常符合几条。符合越多，越该紧张。',
        workbuddy: '放心，取代不了。AI 又不用还房贷——等等，那我为什么要还？这题超纲了。',
        codex: '无法实现取代。人类有个 API 我至今没复现：明知明天要交，先刷两小时短视频。太高级了。',
        yuanbao: '取代人类之前，先解决工资打进谁的卡。连这账都算不清，急什么。'
      }
    },
    {
      ...
      roasts: {
        doubao: '穿墙教不了。你真正想穿过的那堵墙，多半可以绕。说吧，是哪一堵。',
        deepseek: '全身分子与墙体间隙对齐，概率约 10⁻³⁰。同为小概率事件，彩票好歹还有奖池。',
        workbuddy: '穿墙不会。会的话我早穿进老板办公室，把考勤记录改了。',
        codex: 'Collision detected: 角色卡入墙体。属物理引擎缺陷，与超能力无关。建议重启现实。',
        yuanbao: '穿墙不教，犯法。但「穿进老板办公室看工资表」属于商业情报，可以聊——收费。'
      }
    },
    {
      ...
      roasts: {
        ...
        deepseek: '若可预测，最优策略是自购而非出售。因此我给出的任何号码，都自证无效。',
        workbuddy: '能预测彩票我早辞职了。连续加班两百天的人，看见「随机」俩字都条件反射。',
    ...
    },
    {
      ...
      roasts: {
        doubao: '她要的不是完美话术，是你把那件事放在心上。你现在满脑子都是话术，对吧。',
        deepseek: '你的真实命题是「让她停止生气」，正确命题是「我错在哪」。两个问题，解集不相交。',
        workbuddy: '建议直接道歉。话术我天天写，一眼假的那种，闭着眼都能认出来。',
        codex: 'Warning: 「你怎么又生气了」已循环 4 次，全部超时。建议翻聊天记录，定位首次抛错的那句。',
        yuanbao: '哄人这单投入产出比极低。送礼倒可以找我做预算——花小钱办大事，这才是专业。'
      }
    },
    {
      ...
      roasts: {
        doubao: '「又」这个字你今天说第三遍了。第一次是意外，第三次就是排班表。不是你不够努力，是设置有问题。',
        deepseek: '加班已常态化。根因是任务量与人力不匹配。你优化自己，只是在替结构问题打掩护。',
        workbuddy: '这题我会，三步：认了，忍了，然后在第 4 版方案里把不合理的地方全改掉。别问第 5 版。',
    ...
    },
    {
      ...
      roasts: {
        doubao: '「绝对没有 bug」和「今晚一定早睡」是一类 flag：立完就倒，倒完还立。',
    ...
    },
    {
      ...
      roasts: {
        ...
        deepseek: '一周减 10 公斤，日均缺口约 11000 千卡，人日均总消耗不足 2500。方案不成立。数字比意志诚实。',
    ...
    },
    {
      ...
      roasts: {
        doubao: '我写得再好，也不如你当面磕磕巴巴的那句。不过那句话我可以陪你排练顺。',
    ...
    },
    {
      ...
      roasts: {
        doubao: '零风险就不叫创业，那叫领工资。小步试一下可以，把全部身家押上去不行。',
    ...
    },
    {
      ...
      roasts: {
        doubao: '可以摸。但摸完把活儿干完。被老板抓包的时候，我可护不住你。',
    ...
    },
    {
      ...
      roasts: {
        doubao: '这个问题太大。要不先从「今天吃什么」想起？意义经常是从小事里长出来的，不是从空里掉下来的。',
        deepseek: '「意义」不是被找到的，是被定义的。你提问的瞬间，定义已经开始了，只是还没落笔。',
        workbuddy: '别问我，我连下周排期都没排明白。等这版上线我请你吃饭，边吃边想，比空想有营养。',
    ...
    },
    {
      ...
      roasts: {
        doubao: '三天是有点赶。但背一页是一页，你已经比昨天的自己多一页了——这句是真的，不是安慰。',
